...
(function () {
    ...
    function CanvasRenderingContext2D_(canvasTarget, scaledWidth, scaledHeight) {
        ...
        if (!_.has(canvasTarget, "VLines") || !_.isArray(canvasTarget.VLines))
            canvasTarget.VLines = [];
        if (!_.has(canvasTarget, "Polygons") || !_.isArray(canvasTarget.Polygons))
            canvasTarget.Polygons = [];
    ...
    var _drawPDFPolygon = function(points, lineWidth, fillColor, strokeColor) {
        var dashedLine = _.isArray(this.dashArray) && (this.dashArray.length > 1);
        var pP = new PDFPolygon(points, lineWidth, fillColor, strokeColor, dashedLine);
        pP.processPolygon(this.canvas);
    };
            ...
            switch (p.type) {
                ...
                case 'close':
                    if (!aFill) { //lines
                        ...
                        _drawPDFPolygon.call(this, polygonPoints, lineWidth, color, processStyle(this.strokeStyle).color);
                    }
                    p = null;
                    polygonPoints = []
                    break;
